...
/// file. Cgroups missing or unreadable pressure files are skipped
/// (debug-logged) rather than failing the whole snapshot; one stubborn cgroup
/// shouldn't blind us to the other 200.
...
pub fn read_one(cgroup_path: &Path, resource: Resource) -> Result<CgroupPressure> {
  ...
  let parsed = psi::parse(&contents).with_context(|| format!("parsing {}", file.display()))?;
