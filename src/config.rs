...
impl RawConfig {
  fn validate(self) -> Result<Config> {
    ...
    let resource = match self.triggers.resource.as_deref() {
      ...
      Some("io") => Resource::Io,
      Some(other) => {
        bail!("triggers.resource = {other:?}; expected one of \"cpu\", \"memory\", \"io\"")
      }
    };
    ...
    let threshold_ms = self
      .triggers
      .threshold_ms
      .unwrap_or(defaults.triggers.threshold_ms);
    let window_ms = self
      .triggers
      .window_ms
      .unwrap_or(defaults.triggers.window_ms);
    ...
    if !(500..=10_000).contains(&window_ms) {
      bail!("triggers.window_ms = {window_ms}: kernel rejects windows outside 500..=10000");
    }
    if threshold_ms == 0 {
    ...
    for (i, d) in escalate_after.iter().enumerate() {
      ...
      if *d <= prev {
        bail!("policy escalation dwells must be strictly increasing; got {escalate_after:?}");
      }
      prev = *d;
    }
  ...
  for (suffix, factor_ms) in [("ms", 1u64), ("s", 1000), ("m", 60_000), ("h", 3_600_000)] {
    if let Some(num_str) = trimmed.strip_suffix(suffix) {
      let num: u64 = num_str
        .trim()
        .parse()
        .with_context(|| format!("parsing numeric part of duration {trimmed:?} (expected u64)"))?;
...
fn parse_duration_or(maybe: Option<&str>, default: Duration, field: &str) -> Result<Duration> {
  match maybe {
    None => Ok(default),
...
mod tests {
  ...
  fn empty_toml_yields_defaults() {
    ...
    assert_eq!(cfg.triggers.threshold_ms, 100);
    assert_eq!(
      cfg.ladder.escalate_after,
      Config::default().ladder.escalate_after
    );
  }
