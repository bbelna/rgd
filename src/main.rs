...
use rgd::enforce::{EnforceError, EnforcementGates, Enforcer, SystemdBackend};
...
async fn main() -> Result<()> {
  ...
  let trigger =
    Trigger::new(resource, threshold_us, window_us).context("setting up PSI trigger")?;
  ...
  if let Some(enforcer) = enforcer.as_ref() {
    ...
    info!(total_tracked = total, reverted, "teardown complete");
  }
