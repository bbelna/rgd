...
mod tests {
  ...
  fn non_additive_children_saturate_parent_to_zero() {
    ...
    let prev = snap(&[("/p", 0, 0), ("/p/a", 0, 0), ("/p/b", 0, 0)]);
    let cur = snap(&[("/p", 100, 0), ("/p/a", 90, 0), ("/p/b", 90, 0)]);
    let top = rank(&prev, &cur, 5);
  ...
  fn is_direct_child_matches_adjacent_paths_only() {
    assert!(is_direct_child(Path::new("/a/b"), Path::new("/a/b/c")));
    assert!(!is_direct_child(Path::new("/a"), Path::new("/a/b/c")));
