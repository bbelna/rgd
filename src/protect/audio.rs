...
mod tests {
  ...
  fn finds_services_by_exact_basename() {
    ...
    let root = dir.path();
    fs::create_dir_all(root.join("user.slice/user@1000.service/pipewire.service")).unwrap();
    fs::create_dir_all(root.join("user.slice/user@1000.service/wireplumber.service")).unwrap();
    fs::create_dir_all(root.join("user.slice/user@1000.service/unrelated.service")).unwrap();
    let found = discover_under(root);
    assert_eq!(found.len(), 2);
    assert!(found.iter().any(|p| p.ends_with("pipewire.service")));
    assert!(found.iter().any(|p| p.ends_with("wireplumber.service")));
  }
