...
mod tests {
  ...
  fn contains_cgroup_is_exact() {
    ...
    assert!(s.contains_cgroup(Path::new("/sys/fs/cgroup/user.slice/plasma.service")));
    assert!(!s.contains_cgroup(Path::new("/sys/fs/cgroup/user.slice/plasma.service/child")));
  }

  #[test]
  fn covers_is_ancestor_aware() {
    let s = set_with(&["/sys/fs/cgroup/user.slice/plasma.service"]);
    assert!(s.covers(Path::new("/sys/fs/cgroup/user.slice/plasma.service")));
